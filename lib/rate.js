...
export async function allowLogin(req){
 ...
 if(!url||!token){if(process.env.VERCEL)throw Error('Configure o limitador de tentativas (Upstash).');const old=memory.get(id);const v=old&&old.until>Date.now()?old:{n:0,until:Date.now()+900000};v.n++;memory.set(id,v);return v.n<=10}
